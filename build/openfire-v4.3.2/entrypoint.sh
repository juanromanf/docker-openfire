...
initialize_log_dir() {
  ...
  chown -R ${OPENFIRE_USER}:${OPENFIRE_USER} ${OPENFIRE_LOG_DIR} ${OPENFIRE_DIR}/logs
}
...
if [[ -z ${1} ]]; then
  exec start-stop-daemon --start --chuid ${OPENFIRE_USER}:${OPENFIRE_USER} --exec /opt/bitnami/java/bin/java -- \
    -server \
